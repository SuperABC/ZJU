...
HeapNode Original::get() {
	int ret = queue[0]->index;
	HeapNode *tmpNode;
	int tmpIndex = 0;
	int index = 0;

	if (tmpNum == 0)return HeapNode(-1);
	tmpNum--;
	...
	while (1) {
		if (tmpNum > index*2 + 2 && queue[index * 2 + 1]->distance >= queue[index * 2 + 2]->distance && tmpNode->distance > queue[index * 2 + 2]->distance) {
			identifier[queue[index * 2 + 2]->index] = index;
			queue[index] = queue[index * 2 + 2];
			index = index * 2 + 2;
		}
		else if(tmpNum > index * 2 + 1 && (index * 2 + 2) && queue[index * 2 + 1]->distance <= queue[index * 2 + 2]->distance && tmpNode->distance > queue[index * 2 + 1]->distance){
