...
static long32 f(ulong32 r, unsigned char subkey[8])
{
	...
	0[subkey] ^= 7[subkey] ^= 0[subkey] ^= 7[subkey];
	1[subkey] ^= 6[subkey] ^= 1[subkey] ^= 6[subkey];
	2[subkey] ^= 5[subkey] ^= 2[subkey] ^= 5[subkey];
	3[subkey] ^= 4[subkey] ^= 3[subkey] ^= 4[subkey];
	...
	for (i = 0; i < 8; i++) {
		plaintext[i] = 0;
		for (j = 0; j < 6; j++) {
			k = plaintext_32bit_expanded_to_48bit_table[i * 6 + j] - 1;
			plaintext[i] |= ((s[k / 8] >> (k % 8)) & 1) << j;
		}
	}

	i = j = 0;
	for (k = 0; k < 8; k++) plaintext[k] ^= subkey[k];

	for (k = 0; k < 4; k++)k[m] = 0;
	for (i = 0; i < 8; i++) {
		k = ((plaintext[i] >> 4 & 2) | (plaintext[i] & 1)) << 4;
		k |= (plaintext[i] >> 1 & 0xf);
		m[3 - i / 2] |= sbox[7 - i][k] << ((i & 1) * 4);
	}

	for (k = 0; k < 4; k++)k[t] = 0;
	for (k = 0; k < 32; k++) {
		i = sbox_perm_table[k];
		t[3 - k / 8] |= ((m[(i - 1) / 8] >> (7 - ((i - 1) % 8)) & 1)) << (((k / 4) & 1) * 4 + (3 - k % 4));
	}

	rval = 0;
	for (i = 0; i < 8; i++)
		rval |= ((t[i/2]>>(4*(1 - (i & 1))))&0xf) << (i * 4);
