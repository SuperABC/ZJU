...
float para[7] = { .4f, .4f, .1f, .1f, .1f, .1f, .1f };
...
bool gradient(std::vector<glm::vec3> &wi, glm::vec3 &wo, int x, int y, std::vector<float> &brdf, float *res, bool r = false) {
	...
	while (1) {
		...
		light_brdf(x, y, Cam, Light, para, pre);
		...
		para[0] += DELTA;
		light_brdf(x, y, Cam, Light, para, tmp);
		...
		para[3] = clamp(0.0001f, 1.f, para[3]);
		para[4] = clamp(0.0001f, 1.f, para[4]);
